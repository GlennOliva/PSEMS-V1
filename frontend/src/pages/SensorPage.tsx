...
const SensorPage: React.FC<SensorPageProps> = ({ title, unit, sensorType }) => {
  const [dateRange, setDateRange] = useState<'hour' | 'day' | 'week' | 'month'>('week');
  ...
  // ✅ Parse date+time to JS Date
  const parseReadingDate = (r: SensorReading) => new Date(`${r.date}T${r.time}`);

  // ✅ Filter sensorData by hour/day/week/month
  ...
  // ✅ Logs filtered by time range then by status
  const filteredLogs =
    statusFilter === "all"
      ? timeFilteredData
      : timeFilteredData.filter(r => r.status === statusFilter);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            ...
            <select
              value={dateRange}
              onChange={(e) => setDateRange(e.target.value as 'hour' | 'day' | 'week' | 'month')}
